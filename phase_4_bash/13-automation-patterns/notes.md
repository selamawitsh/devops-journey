...
A live example of the OPPOSITE of idempotency: cat >> file (append) is
NOT idempotent. Running it once adds content correctly. Running the same
append command a second time adds the SAME content again, duplicated —
append has no built-in check for "have I already added this," it just
blindly appends every time it's called. This is exactly the kind of bug
idempotency as a design principle is meant to prevent.
